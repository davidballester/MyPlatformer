...
func set_up_character(character_sprite_frames: SpriteFrames, level: Level):
	var character: Character = load("res://scenes/character/PlayableCharacter.tscn").instantiate()
	...
	character.ready.connect(set_up_camera.bind(character, level))
	return character

func position_character(character: PlayableCharacter, level: Level):
...
func set_up_camera(character: PlayableCharacter, level: Level):
	var camera: Camera2D = character.camera
	camera.limit_left = 0
	camera.limit_top = 0
	var level_background: TextureRect = level.get_node("Background")
